...
def train_logistic_regression(x_train, y_train, x_val, y_val, lr):
    ...
    while True:
        for i in range(len(x_train)):
            deriv = (y_train[i] - compute_hypothesis(x_train[i], weights)) * x_train[i]
            weights = weights + lr * deriv
        
        print(f"Train iteration {iteration}:")
        accuracy, precision, recall = evaluate_logistic_regression(x_val, y_val, weights)
...
def evaluate_logistic_regression(x, y, weights):
    x = add_ones(x)
...
def test_logistic_regression():
    x_train, y_train, x_val, y_val, x_test, y_test, feats = read_data("mushrooms_logistic.csv")
    lr = 0.00001  # learning rate

    # train logistic regression
    weights = train_logistic_regression(x_train, y_train, x_val, y_val, lr)
    print(f"Weights: {weights}")

    # test logistic regression
    print(f"Training set:")
    evaluate_logistic_regression(x_train, y_train, weights)

    print(f"Validation set:")
    evaluate_logistic_regression(x_val, y_val, weights)

    print(f"Test set:")
    accuracy, precision, recall = evaluate_logistic_regression(x_test, y_test, weights)

    ranked_feats = rank_features(weights, feats)
    print(f"Top 3 indicative feats for edible mushrooms (y = 0):\n  {ranked_feats[:3]}")
    print(f"Top 3 indicative feats for poisionous mushrooms (y = 1):\n  {ranked_feats[::-1][:3]}")
    print()
    
    return weights, accuracy, precision, recall
...
if iteration == 1:
    ...
    weights_sum = np.zeros(x_train.shape[1] + 1)
    accuracy_sum = 0
    precision_sum = 0
    recall_sum = 0
    ...
    for i in range(iteration):
        ...
        weights, accuracy, precision, recall = test_logistic_regression()
        weights_sum += weights
    ...
    print(f"Average top 3 indicative feats for edible mushrooms (y = 0):\n  {ranked_feats[:3]}")
